...
export interface ArbitrageOpportunity {
  ...
  confidence: number; // 0-1, based on data quality and time to event
  legs: ArbitrageLeg[];
  totalStake: number;
  expectedProfit: number;
  timestamp: number;
  expiresAt: number; // When odds likely to change
  metadata: {
    marketTypes: string[];
...
export interface ArbitrageLeg {
  ...
  contribution: number; // % of total arbitrage profit from this leg
}

export interface CrossMarketMapping {
  sourceMarket: string;
  targetMarket: string;
  conversion: (selection: string, odds: number) => { selection: string; odds: number } | null;
}

export class ArbitrageDetector {
  private readonly MIN_PROFIT_PERCENT = 0.5; // 0.5% minimum
  private readonly MAX_PROFIT_PERCENT = 20; // Cap at 20% (likely error)
  private readonly MIN_CONFIDENCE = 0.3;
  private crossMarketMappings: Map<string, CrossMarketMapping[]> = new Map();
   ...
   */
  private initializeCrossMarketMappings(): void {
   ...
   */
  public detectArbitrage(odds: AggregatedOdds): ArbitrageOpportunity[] {
   ...
   */
  private detectStraightArbitrage(odds: AggregatedOdds): ArbitrageOpportunity[] {
    const opportunities: ArbitrageOpportunity[] = [];
    ...
    for (const [market, selections] of Object.entries(odds.markets)) {
      ...
      if (selectionNames.length < 2) continue;
      ...
      if (bestOdds.size !== selectionNames.length) continue;
   ...
   */
  private detectCrossMarketArbitrage(odds: AggregatedOdds): ArbitrageOpportunity[] {
    const opportunities: ArbitrageOpportunity[] = [];
      ...
      for (const [sourceSelection, sourceBookmakers] of Object.entries(sourceMarket)) {
        for (const [sourceBookmaker, sourceData] of Object.entries(sourceBookmakers)) {
          ...
          for (const [targetBookmaker, targetData] of Object.entries(targetSelection)) {
            ...
            const arbitrageCheck = this.checkCrossMarketArbitrage(
              odds,
              mapping.sourceMarket,
              mapping.targetMarket,
              sourceSelection,
              converted.selection,
              sourceData.odds,
              targetData.odds
   ...
   */
  private detectSyntheticArbitrage(odds: AggregatedOdds): ArbitrageOpportunity[] {
    const opportunities: ArbitrageOpportunity[] = [];
   ...
   */
  private detectMiddleOpportunities(odds: AggregatedOdds): ArbitrageOpportunity[] {
    const opportunities: ArbitrageOpportunity[] = [];

    // Look for totals markets
    const totalsMarket = odds.markets['totals'] || odds.markets['over_under'];
    if (!totalsMarket) return opportunities;

    // Group by line
    const lines: Map<number, { over?: number; under?: number; overBookmaker?: string; underBookmaker?: string }> = new Map();
  ...
  private checkCrossMarketArbitrage(
    ...
    sourceOdds: number,
    targetOdds: number
  ): { isArbitrage: boolean; profitPercent: number; legs: ArbitrageLeg[] } {
   ...
   */
  private calculateConfidence(odds: AggregatedOdds, legs: ArbitrageLeg[]): number {
    let confidence = 1.0;
   ...
   */
  private identifyRiskFactors(odds: AggregatedOdds, legs: ArbitrageLeg[]): string[] {
    const risks: string[] = [];
   ...
   */
  public detectMultiLegArbitrage(
