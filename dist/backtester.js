...
exports.BettingStrategyBacktester = exports.STRATEGIES = void 0;
...
exports.STRATEGIES = {
    fixedStake: {
        ...
        execute: (opportunity, context, params = exports.STRATEGIES.fixedStake.parameters) => {
    ...
    kellyCriterion: {
        ...
        execute: (opportunity, context, params = exports.STRATEGIES.kellyCriterion.parameters) => {
    ...
    percentageOfBankroll: {
        ...
        execute: (opportunity, context, params = exports.STRATEGIES.percentageOfBankroll.parameters) => {
    ...
    martingale: {
        ...
        execute: (opportunity, context, params = exports.STRATEGIES.martingale.parameters) => {
            const multiplier = Math.min(Math.pow(2, context.consecutiveLosses), params.maxMultiplier);
            const stake = params.baseStake * multiplier;
            if (stake > context.currentBankroll * 0.25)
    ...
    valueBased: {
        ...
        execute: (opportunity, context, params = exports.STRATEGIES.valueBased.parameters) => {
            if (opportunity.profitPercent < params.minProfitPercent)
                return null;
            const stake = Math.min(context.currentBankroll * 0.03, params.maxStake);
            return {
                stake,
    ...
    confidenceWeighted: {
        ...
        execute: (opportunity, context, params = exports.STRATEGIES.confidenceWeighted.parameters) => {
...
class BettingStrategyBacktester extends events_1.EventEmitter {
    historicalData = [];
    constructor() {
        super();
    }
    ...
    loadHistoricalData(data) {
        this.historicalData = data.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    }
    /**
    ...
    async runBacktest(config) {
        ...
        const filteredData = this.historicalData.filter(d => {
            ...
            return (date >= config.startDate &&
                ...
                d.bets.some((b) => b.odds >= config.minOdds && b.odds <= config.maxOdds) &&
                (config.sports.length === 0 || config.sports.includes(d.sport)) &&
                (config.bookmakers.length === 0 || d.bets.some((b) => config.bookmakers.includes(b.bookmaker))));
        });
        ...
        for (let i = 0; i < filteredData.length; i++) {
            ...
            if (!decision || decision.stake <= 0)
                continue;
            if (decision.stake > config.maxStakePerBet)
                continue;
            if (decision.stake > bankroll)
                continue; // Can't bet more than we have
        ...
        const sortedDailyPnL = Array.from(dailyPnL.values()).sort((a, b) => a.date.localeCompare(b.date));
     ...
     */
    async compareStrategies(strategies, baseConfig) {
        const results = [];
    ...
    generateReport(result) {
        const lines = [
            ...
            '',
            '### Recent Trades',
            ...result.trades.slice(-5).map(t => `- ${t.timestamp.toISOString().split('T')[0]}: ${t.match} - $${t.stake.toFixed(2)} stake, $${t.profit.toFixed(2)} profit`),
        ];
        return lines.join('\n');
    }
    /**
...
exports.BettingStrategyBacktester = BettingStrategyBacktester;
exports.default = BettingStrategyBacktester;
